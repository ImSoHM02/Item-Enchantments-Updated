	...
	if inst and inst.AnimState and inst.components.modifier == nil and inst.components.combat == nil and inst.components.stackable == nil and inst.components.heavyobstaclephysics == nil then
		...
		inst:DoTaskInTime(0, function()
			...
			if GLOBAL.TheWorld.state.cycles == 0 and GLOBAL.TheWorld.state.time <= 0.01 and not inst.components.modifier:IsModified() and not inst:HasTag("INLIMBO") then--should trigger on world gen
				inst.components.modifier:GenerateFromTable({good=70, rare=25, epic=5}) --on items generated by world,
			end
		end)
	end
end
...
local MIN_INDICATOR_RANGE = (GLOBAL.TUNING.MIN_INDICATOR_RANGE or 10)
local MIN_INDICATOR_RANGE_SQ = MIN_INDICATOR_RANGE * MIN_INDICATOR_RANGE
local BOSS_INDICATOR_MAX_RANGE = (GLOBAL.TUNING.MAX_INDICATOR_RANGE or 50) * 2
local BOSS_INDICATOR_MAX_RANGE_SQ = BOSS_INDICATOR_MAX_RANGE * BOSS_INDICATOR_MAX_RANGE

local function ShouldTrackBossIndicator(inst, viewer)
	if inst == nil or viewer == nil then
		return false
	end
	if not inst:IsValid() or inst:HasTag("INLIMBO") or not viewer:IsValid() then
		return false
	end
	if viewer:HasTag("playerghost") or inst.entity:FrustumCheck() then
		return false
	end
	local distsq = viewer:GetDistanceSqToInst(inst)
	if distsq < MIN_INDICATOR_RANGE_SQ or distsq > BOSS_INDICATOR_MAX_RANGE_SQ then
		return false
	end
	return true
end

local function EnableBossIndicator(inst)
	...
	inst.components.hudindicatable:SetShouldTrackFunction(ShouldTrackBossIndicator)
end

local function TryEnableBossIndicator(inst)
	if inst ~= nil and inst:IsValid() and inst:HasTag("modifier_boss") then
		EnableBossIndicator(inst)
	end
end

local function BuildBossDisplayName(inst)
	local base_name = (inst.prefab and GLOBAL.STRINGS.NAMES[string.upper(inst.prefab)]) or inst.name or inst.prefab or ""
	local rarity_prefix = inst.rarity and GLOBAL.TitleCase(inst.rarity) or nil
	if rarity_prefix ~= nil and rarity_prefix ~= "" then
		return rarity_prefix .. " " .. base_name
	end
	return base_name
end

local function ApplyBossDisplayName(inst, name)
	if inst == nil or name == nil then
		return
	end
	inst.modifierboss_name = name
	inst.displaynamefn = function(ent)
		return ent.modifierboss_name or ent.name or ent.prefab or ""
	end
...
local function InferRarityFromName(name)
	if name == nil or name == "" then
		return nil
	end
	local first = name:match("^%s*(%S+)") or ""
	first = string.lower(first)
	if rarities_lookup[first] then
		return first
	end
	return nil
end

local function EnsureBossNet(inst)
	if inst.modifierboss_rarity_net ~= nil then
		return
	end
	inst.modifierboss_rarity_net = net_string(inst.GUID, "modifierboss.rarity", "modifierbossdirty")
	inst.modifierboss_name_net = net_string(inst.GUID, "modifierboss.name", "modifierbossnamedirty")
	if not GLOBAL.TheWorld.ismastersim then
		local function RefreshBossRarity()
			local rar = inst.modifierboss_rarity_net:value()
			rar = rar ~= "" and string.lower(rar) or nil
			inst.rarity = rar
			if inst.rarity ~= nil then
				TryEnableBossIndicator(inst)
			end
		end
		local function RefreshBossName()
			local name = inst.modifierboss_name_net:value()
			if name ~= nil and name ~= "" then
				if inst.rarity == nil then
					local rar = inst.modifierboss_rarity_net:value()
					inst.rarity = rar ~= "" and string.lower(rar) or nil
					if inst.rarity == nil then
						inst.rarity = InferRarityFromName(name)
					end
				end
				ApplyBossDisplayName(inst, name)
				TryEnableBossIndicator(inst)
			end
		end
...
local function onbuilditem(builder, data)--give modifiers on craft
	-- Check if crafting enchantments are enabled
	if GLOBAL.TUNING.MODIFIER_ENCHANTMENT_SOURCES == "no_craft" then
		return -- Skip crafting enchantments if disabled
	end
	
		if data.item.components.modifier and math.random() <= 0.4 then--40% chance of modifier
			local raritytable = { good = 70, rare = 25, epic = 5 }
			if builder:HasTag("lucky") and not builder:HasTag("unlucky") then--so far nothing in this mod gives these tags, but hopefully some other mods do
				raritytable = { good = 60, rare = 30, epic = 10 }
			end
			if builder:HasTag("unlucky") and not builder:HasTag("lucky") then
				raritytable = { good = 80, rare = 19, epic = 1}
			end
			if data.item.components.modifier_cleaner then
				raritytable = { good = 50, rare = 35, epic = 15 }
			end
	...
	function self:SpawnLootPrefab(lootprefab, pt)
		...
		if loot and loot.components.modifier and not self.inst:HasTag("modifier_boss") and math.random() < 0.25 then--25% of getting a modifier (check if we didnt drop a modifierprefab)
			if self.inst:HasTag("epic") then
				...
				loot.components.modifier:GenerateFromTable({good = 65, rare = 30, epic = 5})
			end
		end
		return loot
	end
...
AddComponentPostInit("playeractionpicker", function(self)
	...
	function self:DoGetMouseActions(position, target, spellbook)--when using mouse to attack - FIXED: Added missing spellbook parameter
		local lmb, rmb = oldDoGetMouseActions(self, position, target, spellbook)
...
AddComponentPostInit("dryingrack", function(self)
	modifier_init(self.inst)
	if self._modifier_ongetitem_wrapped then
		return
	end
	self._modifier_ongetitem_wrapped = true
	local oldOnGetItem = self.OnGetItem
	function self:OnGetItem(item, slot, resumedrytime)
		local dryable = item and item.components and item.components.dryable or nil
		local originaltime = nil
		local mult = self.inst.modifier_drying_speedmult
		if mult and mult > 0 and dryable and dryable.GetDryTime then
			originaltime = dryable:GetDryTime()
			if originaltime ~= nil then
				dryable:SetDryTime(math.max(1, originaltime * mult))
			end
		end
		oldOnGetItem(self, item, slot, resumedrytime)
		if originaltime ~= nil and dryable and item and item:IsValid() then
			dryable:SetDryTime(originaltime)
		end
	end
end)
...
AddComponentPostInit("dryer", function(self)
	modifier_init(self.inst)
	if self._modifier_startdrying_wrapped then
		return
	end
	self._modifier_startdrying_wrapped = true
	local oldStartDrying = self.StartDrying
	function self:StartDrying(dryable)
		local dryablecomp = dryable and dryable.components and dryable.components.dryable or nil
		local originaltime = nil
		local mult = self.inst.modifier_drying_speedmult
		if mult and mult > 0 and dryablecomp and dryablecomp.GetDryTime then
			originaltime = dryablecomp:GetDryTime()
			if originaltime ~= nil then
				dryablecomp:SetDryTime(math.max(1, originaltime * mult))
			end
		end
		local success = oldStartDrying(self, dryable)
		if not success and originaltime ~= nil and dryablecomp and dryablecomp.inst and dryablecomp.inst:IsValid() then
			dryablecomp:SetDryTime(originaltime)
		end
		return success
	end
end)
...
local test_rarities = { "good", "rare", "epic", "legendary", "mythic", "test" }
...
function GLOBAL.SpawnBoss(inst, rarity)
	...
	inst.rarity = rarity
	if inst.modifierboss_rarity_net then
		inst.modifierboss_rarity_net:set(rarity or "")
	end
	local boss_name = BuildBossDisplayName(inst)
	if inst.modifierboss_name_net then
		inst.modifierboss_name_net:set(boss_name or "")
	end
	ApplyBossDisplayName(inst, boss_name)
	TryEnableBossIndicator(inst)
	inst.modifier_boss_color = string.lower(rarity)
